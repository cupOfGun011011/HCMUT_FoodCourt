    ...
    try {
      const res = await axios({
        ...
        headers: {
          ...
          "x-rapidapi-key":
            "19e67bbf91msha57d00648bb06dcp119bcajsneaea3abbee73",
          useQueryString: true,
        },
        params: {
          rId: "37859",
  ...
  updateServings(type) {
    ...
    if (type === "dec") {
      ...
      this.ingredients.forEach((el, i) => {
        newIngredients[i].count = new Fraction(
          newIngredients[i].count.numerator * value.numerator,
          newIngredients[i].count.denominator * value.denominator
        );
    ...
    const newIngredients = this.ingredients.map((el) => {
      ...
      if (parseInt(ingredientArr[0]) || ingredient.includes("/")) {
        ...
        if (ingredientArr[1].includes("/")) {
          ...
          finalIngre.push(
            ingredientArr.slice(3, ingredientArr.length).join(" ")
          );
        } else {
          finalIngre.push(ingredientArr[1]);
          finalIngre.push(
            ingredientArr.slice(2, ingredientArr.length).join(" ")
          );
        }
      } else if (
        unitIndex !== -1 &&
        !(parseInt(ingredientArr[0]) || ingredient.includes("/"))
      ) {
        /// no num but unit
